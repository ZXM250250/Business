...
func (t *AccessControl) Find(search *AccessControl) (*AccessControl, error) {
	model := &AccessControl{}
	err := GetDB().Where(search).Find(model).Error
	return model, err
}

func (t *AccessControl) Save() error {
...
func (t *AccessControl) ListBYServiceID(serviceID int64) ([]AccessControl, int64, error) {
