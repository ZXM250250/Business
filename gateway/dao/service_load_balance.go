...
func (t *LoadBalance) Find(search *LoadBalance) (*LoadBalance, error) {
	model := &LoadBalance{}
	err := GetDB().Where(search).Find(model).Error
	return model, err
}

func (t *LoadBalance) Save() error {
...
func (lbr *LoadBalancer) GetLoadBalancer(service *ServiceDetail) (load_balance.LoadBalance, error) {
	...
	if service.Info.LoadType == tools.LoadTypeTCP || service.Info.LoadType == tools.LoadTypeGRPC {
		schema = ""
	}
