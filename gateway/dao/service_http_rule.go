...
func (t *HttpRule) Find(search *HttpRule) (*HttpRule, error) {
	model := &HttpRule{}
	err := GetDB().Table(t.TableName()).Where(search).Find(model).Error
	return model, err
}

func (t *HttpRule) Save() error {
...
func (t *HttpRule) ListByServiceID(serviceID int64) ([]HttpRule, int64, error) {
	var list []HttpRule
	var count int64
	query := GetDB()
